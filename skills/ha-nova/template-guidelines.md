...
| Contradictory fixed `condition: state` checks on the same entity inside one conjunction | R-20 | The branch is unreachable because the same entity must satisfy mutually exclusive states at the same time | Merge the alternatives under one `condition: or`, or remove the contradictory branch |
